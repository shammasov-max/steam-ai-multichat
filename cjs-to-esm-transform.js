...
module.exports = function transformer(fileInfo, api, options) {
    ...
    // Handle: const X = require("./path") -> import X from "./path"
    ...
    // Handle: module.exports = class X {} -> export default class X {}
    // Handle: module.exports = function() {} -> export default function() {}
    ...
    root.find(j.AssignmentExpression).forEach(path => {
        ...
        assignment.left.property && assignment.left.property.name === 'exports') {
            ...
            if (assignment.right.type === 'ClassExpression') {
                ...
                const className = assignment.right.id || j.identifier('DefaultClass')
                exportDeclaration = j.exportDefaultDeclaration(
                    j.classDeclaration(className, assignment.right.superClass, assignment.right.body, assignment.right.decorators)
                )
            } else if (assignment.right.type === 'FunctionExpression') {
                // module.exports = function name() {} -> export default function name() {}
                exportDeclaration = j.exportDefaultDeclaration(
                    j.functionDeclaration(assignment.right.id, assignment.right.params, assignment.right.body)
                )
            } else if (assignment.right.type === 'ArrowFunctionExpression') {
                // module.exports = () => {} -> export default () => {}
                exportDeclaration = j.exportDefaultDeclaration(assignment.right)
            } else if (assignment.right.type === 'ObjectExpression') {
                // module.exports = { a: 1, b: 2 } -> export { a, b } or export default { a, b }
                // For now, we'll use export default for object expressions
                exportDeclaration = j.exportDefaultDeclaration(assignment.right)
            } else {
            ...
            if (propName) {
                ...
                if (assignment.right.type === 'ClassExpression') {
                    exportDeclaration = j.exportNamedDeclaration(
                        j.classDeclaration(j.identifier(propName), assignment.right.superClass, assignment.right.body)
                    )
                } else if (assignment.right.type === 'FunctionExpression') {
                    exportDeclaration = j.exportNamedDeclaration(
                        j.functionDeclaration(j.identifier(propName), assignment.right.params, assignment.right.body)
                    )
                } else {
                    exportDeclaration = j.exportNamedDeclaration(
