# Dialog AI Service - Implementation Details

This package contains the AI-powered dialog management service. For architecture overview, see the main CLAUDE.md file. This document focuses on implementation-specific details and test organization.
...
- **Files**: `tests/integration/DialogManager.test.ts`, `tests/database/database.test.ts`
...
- **Database Operations**: Database model testing, relationship integrity
...
Tests cover all supported languages: Chinese (zh), Japanese (ja), Korean (ko), English (en), and Spanish (es).

### Mock Services
- **MockOpenAI**: Simulates OpenAI API responses for predictable testing
- **MockRepository**: Database mocking for isolated unit tests
