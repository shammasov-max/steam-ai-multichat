...
export class ContextCompressor {
  ...
  private getRecentMessages(messages: Message[]): CompressedContext['recentMessages'] {
    // When there are many messages, keep only the last few verbatim
    const keepCount = messages.length > this.config.compressionAfterMessages 
      ? this.config.keepLastMessagesVerbatim
      : messages.length
  ...
  private isGreeting(content: string): boolean {
    ...
    // Only consider single-line messages as greetings
    if (trimmedContent.includes('\n')) return false
    // Only very short messages should be considered greetings
    if (trimmedContent.length > 20) return false
    
    return greetings.some(pattern => pattern.test(trimmedContent))
  }
  ...
  private isFarewell(content: string): boolean {
    ...
    const trimmedContent = content.trim()
    // Only consider single-line messages as farewells
    if (trimmedContent.includes('\n')) return false
    // Only very short messages should be considered farewells
    if (trimmedContent.length > 20) return false
    
    return farewells.some(pattern => pattern.test(trimmedContent))
  }
