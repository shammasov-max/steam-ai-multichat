export class LanguageDetector {
  ...
  detect(text: string, fallbackLanguage: string): string {
    ...
    // Check Japanese first (includes hiragana/katakana which are unique to Japanese)
    if (this.languagePatterns.ja.test(trimmedText)) {
      return 'ja';
    }
    
    // Check Korean (Hangul is unique to Korean)
    if (this.languagePatterns.ko.test(trimmedText)) {
      return 'ko';
    }
    
    // Then check Chinese (kanji/hanzi can be in both Chinese and Japanese)
