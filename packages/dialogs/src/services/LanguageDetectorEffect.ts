 ...
 */
export class LanguageDetectorWrapper {
    private service: LanguageDetectorService

    constructor() {
        // Create service instance directly
        this.service = {
            detect: (text: string, fallbackLanguage: string) => {
                return Effect.succeed(detectLanguage(text, fallbackLanguage))
            },
            detectWithConfidence: (text: string, fallbackLanguage: string) => {
            ...
            detectMultiple: (texts: readonly string[], primaryLanguage: string) => {
                const results = Array.from(texts).map(text => detectLanguage(text, primaryLanguage))
                const languageCounts = results.reduce((acc, lang) => {
                    acc[lang] = (acc[lang] || 0) + 1
                    return acc
                }, {} as Record<string, number>)
                const mostFrequent = Object.entries(languageCounts)
                    .sort(([, a], [, b]) => (b as number) - (a as number))[0]
                return Effect.succeed(mostFrequent?.[0] || primaryLanguage) as Effect.Effect<string, LanguageDetectionError, never>
            },
            isLanguageSwitch: (previousLanguage: string, currentText: string) => {
                const detected = detectLanguage(currentText, previousLanguage)
                return Effect.succeed(detected !== previousLanguage)
            }
        }
    }

    detect(text: string, fallbackLanguage: string): string {
        return Effect.runSync(this.service.detect(text, fallbackLanguage))
    }

    detectWithConfidence(text: string, fallbackLanguage: string): { language: string; confidence: number } {
        const language = detectLanguage(text, fallbackLanguage)
        return { language, confidence: 0.8 } // Default confidence
    }

    detectMultiple(texts: string[], primaryLanguage: string): string {
        const results = texts.map(text => detectLanguage(text, primaryLanguage))
        const languageCounts = results.reduce((acc, lang) => {
            acc[lang] = (acc[lang] || 0) + 1
            return acc
        }, {} as Record<string, number>)
        const mostFrequent = Object.entries(languageCounts)
            .sort(([, a], [, b]) => (b as number) - (a as number))[0]
        return mostFrequent?.[0] || primaryLanguage
    }

    isLanguageSwitch(previousLanguage: string, currentText: string): boolean {
        return Effect.runSync(this.service.isLanguageSwitch(previousLanguage, currentText))
    }
}
