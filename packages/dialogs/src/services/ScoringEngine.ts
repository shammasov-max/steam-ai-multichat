...
export class ScoringEngine {
  ...
  async evaluateDialog(
    ...
    const factors = this.calculateFactors(messages, goal, init)
    const continuationScore = this.calculateScore(factors)
    const trend = this.determineTrend(continuationScore)
    const goalProgress = this.calculateGoalProgress(messages, init)
  ...
  private calculateFactors(
    ...
    const goalProximity = this.calculateGoalProximity(messages)
  ...
  private calculateGoalProximity(messages: Message[]): number {
    const userMessages = messages.filter(m => m.role === 'USER')
    
    // Simple heuristic based on conversation progress
  ...
  private calculateGoalProgress(
    ...
    const proximityScore = this.calculateGoalProximity(messages)
  ...
  private determineTrend(currentScore: number): 'rising' | 'stable' | 'declining' {
    // For the first score, check if it's high/low enough to indicate trend
    if (this.previousScores.length === 0) {
      if (currentScore > 0.7) return 'rising'
      if (currentScore < 0.3) return 'declining'
      return 'stable'
    }
    
    if (this.previousScores.length < 2) {
      // Compare with the single previous score
      const difference = currentScore - (this.previousScores[0] ?? 0)
