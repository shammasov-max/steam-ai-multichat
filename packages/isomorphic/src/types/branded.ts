 ...
 * Re-exported from accounts.ts for convenience
 */
import { SteamID64 as SteamID64Schema } from '../slices/accounts'
 ...
 */
export const createAccountId = (steamId64: string): AccountId =>
    `account_${steamId64}` as AccountId
...
export const isAccountId = (value: string): value is AccountId =>
    /^account_[\w-]+$/.test(value)

/**
 * Check if a string is a valid Dialog ID
 */
export const isDialogId = (value: string): value is DialogId =>
    /^dialog_[\w-]+$/.test(value)

/**
 * Check if a string is a valid System ID
 */
export const isSystemId = (value: string): value is SystemId =>
    value === 'system'