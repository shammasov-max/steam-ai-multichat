    ...
    load(): Effect.Effect<unknown, ConfigError, never> {
        return Effect.try({
            try: () => ({
                ...
                database: {
                    connectionString: process.env.DATABASE_URL || process.env.MONGODB_URL || 'mongodb://localhost:27017/effect_redux_db',
                ...
                rateLimit: {
                    friendInvites: {
                        perMinute: parseInt(process.env.RATE_LIMIT_FRIEND_INVITES_PER_MINUTE || '1', 10),
                        perAccount: parseInt(process.env.RATE_LIMIT_FRIEND_INVITES_PER_ACCOUNT || '60', 10)
                    },
                    apiRequests: {
...
export const getConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getFullConfig())
)

export const getDatabaseConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getDatabase())
)

export const getOpenAIConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getOpenAI())
)

export const getScoringConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getScoring())
)

export const getContextConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getContext())
)

export const getServerConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getServer())
)

export const getRateLimitConfig = ConfigService.pipe(
    Effect.flatMap(_ => _.getRateLimit())
)

export const getEnvironment = ConfigService.pipe(
    Effect.flatMap(_ => _.getEnvironment())
)