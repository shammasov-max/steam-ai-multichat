...
* **ID policy**: entity IDs are generated with **`typeid-js`**, prefixed by slice name (`account_*`, `task_*`, `dialog_*`).
...
@app/isomorphic/
  events/
    core.ts       # Branded primitives & aggregate refs (SteamID64, AccountId, DialogId, TaskId, AggregateRef)
...
* `AccountId`, `DialogId`, `TaskId` (non-empty strings).
* `AggregateRef = { type: 'Account'|'Dialog'|'Task'|'System', id: string }`.
...
* Accounts: `"account.connected"`, `"account.disconnected"`, `"account.authenticationFailed"`.
...
* Dialogs: `"dialog.started"`, `"dialog.messageReceived"`, `"dialog.messageSent"`, `"dialog.agentToggled"`.
...
type EventMeta = {
  ...
  aggregate: { type: 'Account'|'Dialog'|'Task'|'System'; id: string };
...
```ts
export const dialogMessageSent = createEventAction(
  "dialog.messageSent",
  p => ({ type: "Dialog", id: p.dialogId })
);
```

Use these creators:

* **Server**: `const evt = dialogMessageSent.make({ dialogId, text })`.
* **Client**: `const evt = dialogMessageSent.decodeIncoming(raw)`.
...
event: batch
data: {"events":[
  {"type":"dialog.messageSent","payload":{"dialogId":"...","text":"..."},"meta":{...}},
  {"type":"dialog.agentToggled","payload":{"dialogId":"...","enabled":true},"meta":{...}}
]}
...
```ts
import { dialogMessageSent } from "@app/isomorphic/events/actions";
import type { WireAction } from "@app/isomorphic/events/meta";

// 1) Build a validated event
const evt: WireAction = dialogMessageSent.make({ dialogId: "dialog_01", text: "Hello!" });
...
* Generate entity ids (accounts/tasks/dialogs) using **`typeid-js`** (e.g., `dialog_...`).
...
```ts
"dialog.typing": S.Struct({ dialogId: DialogId, isTyping: S.Boolean })
```

2. **Create action** in `actions.ts`:

```ts
export const dialogTyping = createEventAction(
  "dialog.typing",
  p => ({ type: "Dialog", id: p.dialogId })
);
```
...
```ts
builder.addCase(dialogTyping.type as any, (state, action: any) => {
  // update slice
});
```
...
* Prefer **normalized** state (by id) for large collections (`dialogs`, `tasks`).
* Limit heavy history in initial **snapshot** (e.g., last `N` messages per dialog).
