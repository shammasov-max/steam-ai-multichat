// TODO: This file needs migration to Effect-based API - createSteamAgent was removed
// import { createSteamAgent } from '../../src/index.js';
import { readFile } from 'fs/promises'
import { join } from 'path'

// Temporary stub for legacy API - this entire file needs rewriting
const createSteamAgent = (_config: any) => ({}) as any
...
class TestAgentPool {
    ...
    async initialize(): Promise<void> {
        ...
        if (accounts.length < 9) {
            throw new Error(
                `Need at least 9 test accounts to use indices 6,7,8, found ${accounts.length}`
            )
        }
    ...
    async loginAll(): Promise<void> {
        ...
        for (let i = 0; i < agents.length; i++) {
            ...
            if (pooledAgent.loginAttempts > 0 && timeSinceLastAttempt < this.LOGIN_RETRY_DELAY) {
                ...
                console.log(
                    `⏳ Skipping ${pooledAgent.account.login} (waiting ${waitTime}s before retry)`
                )
                continue
            }
            ...
            this.loginQueue = this.loginQueue.then(async () => {
                try {
                    console.log(
                        `Attempt ${pooledAgent.loginAttempts + 1}: Logging in ${pooledAgent.account.login}...`
                    )
                    ...
                    console.log(
                        `✅ ${pooledAgent.account.login} logged in with SteamID: ${pooledAgent.steamID}`
                    )
                    successCount++
                    ...
                    if (i < agents.length - 1) {
                        console.log(
                            `⏳ Waiting ${this.LOGIN_QUEUE_DELAY / 1000}s before next login...`
                        )
                        await new Promise(resolve => setTimeout(resolve, this.LOGIN_QUEUE_DELAY))
                    }
                } catch (error: any) {
                    console.error(
                        `❌ Failed to login ${pooledAgent.account.login}: ${error.message}`
                    )
                    ...
                    if (
                        ...
                        console.log(
                            `⚠️  Rate limited on ${pooledAgent.account.login}, will retry later`
                        )
                    }
                }
            })
