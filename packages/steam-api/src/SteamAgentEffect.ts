...
export class SteamConnectionService extends Context.Tag('SteamConnection')<
...
const createSteamClient = (config: SteamAgentConfig) =>
    Effect.gen(function* () {
        ...
        client.chat.on('friendTyping', (msg: any) => {
            Queue.unsafeOffer(events, {
                _tag: 'FriendTyping',
                steamId: msg.steamid_friend.toString(),
            })
        })
    ...
    Effect.gen(function* () {
        ...
        yield* Effect.addFinalizer(() =>
            Effect.gen(function* () {
                ...
                yield* Effect.ignore(
                    logger.info('Steam disconnected', { steamId: connection.steamId })
                )
            })
        )
...
export const createSteamLayer = (config: SteamAgentConfig) => {
    ...
    return Layer.mergeAll(configLayer, connectionLayer, operationsLayer)
}
...
export const runWithSteamPool = <R, E, A>(program: Effect.Effect<A, E, SteamConnectionPool | R>) =>
