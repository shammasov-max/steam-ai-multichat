...
export async function loadTestAccounts(): Promise<TestAccount[]> {
    // Use fixtures directory in root (relative to this file location)
    const fixturesPath = path.join(__dirname, '../../../../fixtures/all.txt')
    ...
    for (const line of content.split('\n')) {
        ...
        const maFilePath = path.join(__dirname, '../../../../fixtures/mafile', `${login}.maFile`)
...
export function parseTestAccounts(): ParsedAccount[] {
    ...
    content.split('\n').forEach(line => {
        ...
        if (match) {
            ...
            const maFilePath = path.join(__dirname, '../../../../fixtures/mafile', `${username}.maFile`)
            if (fs.existsSync(maFilePath)) {
